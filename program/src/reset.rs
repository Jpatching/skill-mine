...
pub fn process_reset(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    ...
    round_next_info
        .is_empty()?
        .is_writable()?
        .has_seeds(&[ROUND, &(board.round_id + 1).to_le_bytes()], &skill_api::ID)?;
    ...
    let round_next = round_next_info.as_account_mut::<Round>(&skill_api::ID)?;
